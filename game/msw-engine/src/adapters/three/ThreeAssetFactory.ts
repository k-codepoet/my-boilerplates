...
import type { ThreeAudio } from "./ThreeAudio";

export class ThreeAssetFactory implements AssetFactorySubsystem {
  private readonly audio: ThreeAudio;

  constructor(audio: ThreeAudio) {
    this.audio = audio;
  }
  ...
  async createTexture(desc: TextureDescriptor): Promise<EngineTexture> {
    ...
    return { id: desc.id, width, height, frames, fps: desc.fps };
  }
  ...
  async createStateMap(desc: StateMapDescriptor): Promise<EngineStateMap> {
    ...
    return { id: desc.id, states, defaultState: desc.defaultState };
  }
  ...
  async createAudio(desc: AudioDescriptor): Promise<EngineAudio> {
    ...
    return { id: desc.id, buffer, category: desc.category };
  }
  ...
  async createAtlas(desc: AtlasDescriptor): Promise<EngineAtlas> {
    ...
    return { id: desc.id, image, regions };
  }
}
