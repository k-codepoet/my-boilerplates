...
class GameGlueRenderScene extends Phaser.Scene {
  constructor() {
    super({ key: "GameGlueRenderScene" });
  }
}
...
export class PhaserRenderer {
  ...
  init(canvas: HTMLCanvasElement, width: number, height: number): void {
    ...
    this.phaserGame = new Phaser.Game({
      ...
      scene: GameGlueRenderScene,
    });

    // Get scene reference once ready
    this.phaserGame.events.on("ready", () => {
      const scene = this.phaserGame!.scene.getScene("GameGlueRenderScene");
      if (scene) {
        this.scene = scene;
  ...
  private registerTextures(bitmaps: ImageBitmap[]): string[] {
    ...
    for (const bitmap of bitmaps) {
      const key = `__gg_tex_${this.textureCounter++}`;
