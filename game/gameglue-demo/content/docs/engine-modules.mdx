---
title: 엔진 코어 모듈
description: Game, Scene, GameObject, GameLoop 등 핵심 모듈 상세
---
...
GameObject 생성 시 `Transform`의 `x`, `y`, `rotation`, `scale.x`, `scale.y`는 JavaScript 객체가 아니라 공유 `Float32Array`에 직접 읽기/쓰기하는 Proxy로 구현된다. 상세 내용은 [메모리 최적화](/docs/memory-optimization) 참조.
...
구체적인 Trait 구현들은 [Trait 시스템](/docs/trait-system) 참조.
